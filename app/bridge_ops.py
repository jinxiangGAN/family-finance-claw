...
def main() -> None:
    ...
    if args.command == "snapshot":
        print(json.dumps(_snapshot(args.user_id), ensure_ascii=False))
        return

    if args.command == "skill":
        params = json.loads(args.params_json or "{}")
        result = execute_skill(args.name, args.user_id, args.user_name, params)
    ...
    if args.command == "store-memory":
        ...
        print(json.dumps({"success": True, "memory_id": memory_id}, ensure_ascii=False))
        return
